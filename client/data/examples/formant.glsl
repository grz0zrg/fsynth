  ...
  // Note :
  //   Must add/select "Subtractive" instrument in "SYNTH" to the first part of the canvas  and assign second output channel. (Korg 35 LPF filter type can also sound quite good!)
  ...
  void main () {
    ...
    if (uv.x > 0.5) {
      ...
      const float frequencies[2] = float[2](1.125, 1.5);
      
      float base_frequency = 480. + sin(globalTime) * 8.;
      ...
      for (int i = 0; i < frequencies.length(); i += 1) {
        ...
        float impulse_response_decay_time = 0.00009; // decay time of the impulse (seconds)
