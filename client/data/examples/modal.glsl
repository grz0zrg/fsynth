  // Sample program : modal synthesis with subtractive noise source
  // Note :
  //   Must add/select "Subtractive" instrument in "SYNTH" to the first part of the canvas.
  //   Must add/select "Modal" instrument in "SYNTH" to the second part of the canvas.
  ...
  void main () {
    ...
    if (uv.x > 0.5) {
      ...
      const float frequencies[5] = float[5](1., 2.32, 4.25, 6.63, 9.38);
      ...
      for (int i = 0; i < frequencies.length(); i += 1) {
        ...
        float filter_q_factor = 120.; // Q factor of the resonant filter
        
      	l += fline(frequency) * 0.125;
      	r += fline(frequency) * 0.125;
        b += fline(frequency) * (source_index + source_mode);
        a += fline(frequency) * filter_q_factor;
      }
    }
